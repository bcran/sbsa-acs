...
function build_with_NIST()
{
    ...
    build -a AARCH64 -t GCC5 -b NOOPT -p ShellPkg/ShellPkg.dsc -m ShellPkg/Application/sbsa-acs/uefi_app/SbsaAvsNist.inf -D ENABLE_NIST
    status=$?
    if [ $status -ne 0 ]; then
...
if [ $NISTStatus -ne 0 ]; then
    build -a AARCH64 -t GCC5 -b NOOPT -p ShellPkg/ShellPkg.dsc -m ShellPkg/Application/sbsa-acs/uefi_app/SbsaAvs.inf
fi
